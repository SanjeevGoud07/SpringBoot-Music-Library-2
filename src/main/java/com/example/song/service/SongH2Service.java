...
public class SongH2Service implements SongRepository{
    ...
    public ArrayList<Song> getSongs(){
        // List<Song> songList=jb.query("select * from playlist",new SongRowMapper());
        // songs=new ArrayList<>(songsList);
    ...
    public Song addSong(Song song){
        ...
        // return db.queryForObject("select * from book where songName=? and lyricist=? and singer=? and musicDirector=?",new SongRowMapper(),song.getSongName(),song.getLyricist(),song.getSinger(),song.getMusicDirector());
    }
    ...
    public String addBulkList(ArrayList<Song> listed){
        int S=0;
        for(Song E:listed){
                db.update("insert into playlist(songName,lyricist,singer,musicDirector) values(?,?,?)",E.getSongName(),E.getLyricist(),E.getSinger(),E.getMusicDirector());
            S+=1;
        }
        String Res="Successfully added "+S+" songs";
        return Res;
    }
}